"""
# DDC Phase 0
# Tests Stage A, B, and C as per the Phase 0 Execution Order document.
"""
import os
import json
import torch
...
def run_stage_b(seed: int, dir: str):
    ...
    for key in ['X_traj', 'P_traj', 'Z_traj', 'N_traj']:
        assert torch.equal(traj1[key], traj2[key]), f"[ERROR] Trajactory '{key}' differs between runs!"
        print(f'Check 2 passed: All trajectory (X/P/Z/N) are element-wise identical.')

    print('Stage B passed. RNG isolation is robust.')
...
def run_stage_c(seed: int, dir: str):
    ...
    print(f'Stage C passed. M={M} dataset saved to {save_path}')
